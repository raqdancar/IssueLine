// Gestiona proveidors i textos localitzats de la interfície.
const messages = {
  common: {
    appName: 'IssueLine',
    account: 'Account',
    signIn: 'Sign in',
    signUp: 'Sign up',
    signOut: 'Sign out',
    save: 'Save',
    cancel: 'Cancel',
    loading: 'Loading...',
    error: 'Error',
    close: 'Close',
    unknown: 'Unknown',
    issue: 'Issue',
    issueCover: 'Issue cover',
  },
  header: {
    openMenu: 'Open menu',
    closeMenu: 'Close menu',
    chooseLanguage: 'Choose language',
    languageSpanish: 'Spanish',
    languageCatalan: 'Catalan',
    languageEnglish: 'English',
    userAvatarAlt: 'User avatar',
    noActiveSession: 'No active session',
    signInSignUp: 'Sign in / Sign up',
  },
  app: {
    configureSupabaseToLoadHeroes: 'Configure Supabase to load heroes.',
    loadingHeroes: 'Loading heroes...',
    loadedHeroes: 'Loaded {{count}} heroes.',
    noHeroesFound: 'No heroes found.',
    emailPasswordRequired: 'Email and password are required.',
    validatingCredentials: 'Validating credentials...',
    signedInAs: 'Signed in as {{email}}',
    passwordsDoNotMatch: 'Passwords do not match.',
    creatingAccount: 'Creating account...',
    accountCreatedCheckInbox: 'Account created. Check your inbox to confirm your email.',
    signedOut: 'Signed out.',
    heroVisualizer: 'Hero visualizer',
    showingCuratedHeroes: 'Showing {{count}} curated heroes stored in Supabase.',
    connectSupabaseAndSeed: 'Connect Supabase and seed your roster to view it here.',
    noHeroesSeedHint: 'No heroes found. Run npm run seed:superheroes to populate the cache.',
    pendingEnvVars: 'Pending environment variables',
    envVarsHint:
      'Copy .env.example to .env and add VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY values.',
  },
  authVerified: {
    eyebrow: 'Account verified',
    title: 'Email verified successfully',
    subtitle: 'Your IssueLine account is now active.',
    body: 'You can now sign in, save your collection, mark readings, and explore editorial timelines.',
    signInCta: 'Sign in',
    homeCta: 'Back to homepage',
    errorTitle: 'Email verification failed',
    errorSubtitle: 'The verification link could not be completed.',
  },
  storageNotice: {
    ariaLabel: 'Local storage notice',
    title: 'We use necessary local storage',
    body: 'IssueLine only uses necessary local storage. We do not collect personal data for analytics, advertising, profiling, or tracking. If you create an account, Supabase uses your email and session only to authenticate you.',
    accept: 'Got it',
    learnMore: 'View policy',
  },
  privacy: {
    eyebrow: 'Privacy and storage',
    title: 'Privacy and local storage policy',
    intro: 'IssueLine uses browser local storage only for technical features and preferences. We do not collect personal data for analytics, advertising, profiling, data sale, or commercial tracking.',
    personalDataTitle: 'No personal data collection for tracking',
    personalDataBody: 'IssueLine does not create user profiles, does not share data with advertisers, and does not measure your activity with third-party tools. The only account-linked information is what Supabase Auth needs to sign you in, keep your session active, and protect your actions.',
    storageTitle: 'What is stored',
    storageItemAuth: 'Supabase Auth session: keeps you signed in and protects user actions.',
    storageItemLocale: 'Selected language: remembers whether you prefer Catalan, Spanish, or English.',
    storageItemNotice: 'Storage notice: remembers that you have already seen this notice.',
    noTrackingTitle: 'No analytics, marketing, or profiling',
    noTrackingBody: 'No Google Analytics, pixels, PostHog, Vercel Analytics, advertising cookies, or profiling tools were found in the app.',
    manageTitle: 'How to manage it',
    manageBody: 'You can delete this data from your browser settings by clearing site data. When you sign out, Supabase removes the active session from the app.',
    backHome: 'Back to homepage',
  },
  dataSources: {
    eyebrow: 'Data sources and licenses',
    title: 'Data sources and cover-use notes',
    intro:
      'IssueLine combines manually curated archive data with comic metadata sourced in part from the Grand Comics Database.',
    gcdTitle: 'Grand Comics Database metadata',
    gcdBody:
      'Comic metadata is sourced in part from the Grand Comics Database (GCD). GCD database portions are licensed under Creative Commons Attribution-ShareAlike 4.0 International.',
    coversTitle: 'Cover thumbnails',
    coversBody:
      'Cover thumbnails are used only to identify comics and issues. Rights to cover artwork remain with the respective copyright holders.',
    independentTitle: 'Independent archive',
    independentBody:
      'IssueLine is an unofficial archival project and is not affiliated with, sponsored by, or endorsed by GCD, publishers, creators, or rights holders.',
    linksTitle: 'Reference links',
    gcdLink: 'Grand Comics Database',
    ccLink: 'CC BY-SA 4.0',
    ccBadgeAlt: 'Creative Commons Attribution-ShareAlike 4.0 license badge',
    backHome: 'Back to homepage',
  },
  footer: {
    ariaSiteFooter: 'Site footer',
    ariaFooterLinks: 'Footer links',
    tagline: 'An editorial archive for timelines, collected editions, and the long memory of comic book continuity.',
    legal: '© 2026 IssueLine',
    disclaimer: 'Unofficial archival project. Data and artwork belong to their respective owners.',
    gcdAttribution:
      'Comic metadata is sourced in part from the Grand Comics Database under CC BY-SA 4.0. Cover thumbnails are used for identification only.',
    about: 'About',
    feedback: 'Feedback',
    privacy: 'Privacy',
    roadmap: 'Roadmap',
    dataSources: 'Data sources',
    changelog: 'Changelog',
    github: 'GitHub',
  },
  home: {
    hero: {
      eyebrow: 'Editorial timelines for collectors',
      title: 'Explore decades of publishing history as a living collection.',
      subtitle:
        'Follow runs, eras, covers, gaps, collected editions, and reading progress through visual timelines built for comic archives.',
      primaryCta: 'Explore timeline',
      secondaryCta: 'View timeline preview',
      scrollHint: 'Enter the archive',
    },
    showcase: {
      eyebrow: 'Character archives',
      title: 'Heroes presented as editorial histories.',
      description:
        'Each archive brings together active years, mapped issues, collected formats, and stage-by-stage progress without reducing the experience to a database table.',
      years: 'Active years',
      issues: 'Issues',
      stages: 'Stages',
      collected: 'Collected',
      completion: 'Indexed',
      cta: 'Explore timeline',
      noteTimeline: 'Chronological coverage from first appearances to modern relaunches.',
      noteStages: 'Runs and editorial eras grouped into readable stages.',
      noteEditions: 'Prepared for omnibus, TPB, deluxe, and regional collected editions.',
    },
    timelinePreview: {
      eyebrow: 'Timeline preview',
      title: 'A vertical archive where every cover has a place in time.',
      description:
        'The Home now foregrounds the core product experience: a readable timeline that makes eras, missing issues, completion states, and collected context visible at a glance.',
      calloutCovers: 'Covers as anchors',
      calloutGaps: 'Missing issues surfaced clearly',
      calloutStages: 'Editorial eras grouped visually',
      calloutProgress: 'Owned and read status ready',
      demoLabel: 'Demo archive',
      demoTitle: 'Doctor Strange continuity map',
      liveFeeling: 'Interactive feel',
      stageCoverage: 'Stage coverage',
      entries: {
        origin: 'First mystical appearances',
        solo: 'Solo series opens the archive',
        missingArc: 'Gap detected in the run',
        legacy: 'Legacy numbering restored',
        modern: 'Modern relaunch tracked',
      },
      summaries: {
        origin: 'A real timeline-style issue row with cover, date, series metadata, and collection state.',
        solo: 'The card keeps publication order visible while separating owned and read status.',
        missingArc: 'Missing issues appear as timeline events, so gaps are visible inside the run.',
        legacy: 'Legacy numbering can sit beside modern metadata without hiding the issue context.',
        modern: 'Recent relaunches remain connected to the same editorial archive.',
      },
      stageNames: {
        origin: 'Origins',
        solo: 'Solo run',
        missingArc: 'Gap',
        legacy: 'Legacy',
        modern: 'Modern',
      },
      states: {
        complete: 'Complete stage',
        owned: 'Owned issues',
        missing: 'Missing issues',
        read: 'Read arc',
        tracking: 'Tracking now',
      },
    },
    how: {
      eyebrow: 'How it works',
      title: 'From shelf to timeline in four focused actions.',
      description:
        'IssueLine keeps collector workflows simple while preserving the editorial context that  matters in  each issue.',
      steps: {
        track: {
          title: 'Track collection',
          body: 'Mark what you own across singles and collected formats without losing the publication order.',
        },
        read: {
          title: 'Mark as read',
          body: 'Separate ownership from reading progress so every run can become a guided queue.',
        },
        missing: {
          title: 'Discover gaps',
          body: 'Spot missing issues, skipped eras, and incomplete stages before hunting for the next edition.',
        },
        history: {
          title: 'Explore history',
          body: 'Move through decades of continuity by year, stage, issue, and editorial milestone.',
        },
      },
    },
    collectors: {
      eyebrow: 'Built for collectors',
      title: 'Gives the feeling of opening an archive box.',
      description:
        'The experience should feel nostalgic, precise, and alive: part museum catalog, part reading map, part collector checklist.',
      quote:
        'A good timeline does not just say what exists. It tells you what belongs together, what is missing, and where the next discovery starts.',
      items: {
        missing: {
          title: 'Identify missing issues instantly',
          body: 'Gaps stand out in context instead of hiding inside filtered lists.',
        },
        runs: {
          title: 'Track complete runs',
          body: 'Group long publication histories into meaningful runs and eras.',
        },
        progress: {
          title: 'Organize reading progress',
          body: 'Keep read, owned, and wanted states visible as part of the timeline.',
        },
        continuity: {
          title: 'Explore decades of continuity',
          body: 'Move between old numbering, relaunches, specials, and collected reprints.',
        },
      },
    },
    stats: {
      heroes: 'Heroes archived',
      issues: 'Issues indexed',
      stages: 'Editorial stages',
      collectedEditions: 'Collected editions',
    },
    editions: {
      eyebrow: 'Collected editions',
      title: 'Singles are only one part of the shelf.',
      description:
        'IssueLine shows omnibus, trade paperbacks, deluxe editions, Marvel Gold style lines, and future collected-format coverage beside the timeline.',
      note: 'Static showcase today, dynamic collected-edition mapping tomorrow.',
      cta: 'See timeline context',
      items: {
        omnibus: { title: 'Omnibus', meta: 'Oversized runs' },
        tpb: { title: 'TPBs', meta: 'Reading arcs' },
        gold: { title: 'Marvel Gold', meta: 'Classic eras' },
        deluxe: { title: 'Deluxe', meta: 'Premium shelf' },
      },
    },
  },
  authDialog: {
    closeDialog: 'Close dialog',
    accountAccess: 'Account access',
    title: 'Sign in or create an account',
    subtitle: 'Use your Supabase credentials to sync reading progress in IssueLine.',
    modeSignIn: 'Sign in',
    modeSignUp: 'Create account',
    supabaseNotConfigured: 'Supabase is not configured.',
    supabaseConfigHint:
      'Add VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY in .env to enable authentication.',
    email: 'Email',
    password: 'Password',
    confirmPassword: 'Confirm password',
    emailPlaceholder: 'ada@example.com',
    passwordPlaceholder: '********',
    repeatPasswordPlaceholder: 'Repeat password',
    signingIn: 'Signing in...',
    creatingAccount: 'Creating account...',
    signUpEmailConfirmationHint:
      'Accounts require email confirmation. After signing up, check your inbox for a confirmation email to activate your account.',
  },
  heroTab: {
    viewDetailsFor: 'View details for {{name}}',
    aka: 'aka {{name}}',
    independent: 'Independent',
    noIssuesBadge: 'No issues',
    noIssuesTooltip: 'This hero does not have any associated issues yet.',
    noStatValue: '--',
  },
  heroDetail: {
    missingSlug: 'Missing hero slug in the URL.',
    heroNotFound: 'No hero found with slug "{{slug}}".',
    heroPortrait: 'Hero portrait',
    sectionTitle: 'Hero timeline',
    backToRoster: 'Back to homepage',
    supabaseHint: 'Configure VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY to view hero details.',
    loadingHeroData: 'Loading hero data...',
    spotlightTitle: 'Hero spotlight',
    spotlightP1:
      "Following a character's history should not feel like solving a puzzle. For decades, heroes like Doctor Strange have lived through relaunches, variants, crossovers, and region-specific editions.",
    spotlightP2:
      'What used to be one flagship run turns into paperbacks, facsimiles, or issues inside larger events, leaving a fragmented timeline.',
    spotlightResult: 'The result?',
    spotlightP3:
      'A confusing reading order that is hard to follow, harder to catalog, and nearly impossible to collect with confidence.',
    spotlightBullet1: 'Which issue actually contains this chapter?',
    spotlightBullet2: 'Is this a new story or another reprint of the same material?',
    spotlightBullet3: 'In what order should you experience every beat?',
    spotlightClosing:
      'IssueLine brings back clarity with one organized timeline so you can enjoy every chapter without getting lost in decades of publications.',
    archiveEyebrow: 'Character archive',
    archiveTitle: 'Timeline briefing',
    archiveSummary:
      '{{heroName}} is tracked through {{issueCount}} issues across {{stageCount}} editorial stages, from first indexed appearances to the latest logged releases.',
    archiveBackendDisabled: 'Configure VITE_BACKEND_URL to show this character archive.',
    archiveLoadError: 'Unable to load the character archive.',
    archiveEmpty: 'This character archive has no indexed issues yet.',
    archiveNoCovers: 'Covers pending',
    archiveYears: 'Years',
    archiveIssues: 'Issues',
    archiveStages: 'Stages',
    archiveCollected: 'Collected',
    archiveFirst: 'First indexed',
    archiveLatest: 'Latest indexed',
    archiveReadProgress: 'Read progress',
    archiveOwnedProgress: 'Owned progress',
    archiveRoute: 'Editorial route',
    alignment: 'Alignment: {{value}}',
    fullName: 'Full name:',
    origin: 'Origin:',
    occupation: 'Occupation:',
    timelineStyle: 'Timeline style',
    switchLayouts: 'Switch between layouts',
    classic: 'Classic',
    cinematic: 'Cinematic',
    selectHeroHint: 'Select a hero from the roster to view their timeline.',
    noStatValue: '--',
    stats: {
      intelligence: 'Intelligence',
      strength: 'Strength',
      speed: 'Speed',
      durability: 'Durability',
      power: 'Power',
      combat: 'Combat',
    },
  },
  account: {
    title: 'Account settings',
    subtitle: 'Manage your display name, private avatar, and password.',
    signInHint: 'Sign in first to manage your profile, avatar, and password.',
    supabaseHint: 'Configure VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY to manage user settings.',
    backToRoster: 'Back to homepage',
    signIn: 'Sign in',
    profile: 'Profile',
    displayName: 'Display name',
    displayNamePlaceholder: 'Stephen Strange',
    saveProfile: 'Save profile',
    savingProfile: 'Saving...',
    profileUpdated: 'Profile updated successfully.',
    avatarUpload: 'Avatar upload',
    uploadFromDevice: 'Upload from your device',
    avatarRules: 'Allowed: JPG, PNG, WEBP, GIF. Max: 5MB.',
    uploadAvatar: 'Upload avatar',
    uploadingAvatar: 'Uploading...',
    deleteAvatar: 'Delete avatar',
    deletingAvatar: 'Deleting...',
    noAvatar: 'No avatar',
    email: 'Email',
    password: 'Password',
    newPassword: 'New password',
    confirmNewPassword: 'Confirm new password',
    passwordPlaceholder: 'At least 8 characters',
    repeatPasswordPlaceholder: 'Repeat your new password',
    updatePassword: 'Update password',
    updatingPassword: 'Updating...',
    avatarPreviewAlt: 'Avatar preview',
    avatarPreviewError: 'Avatar preview error: {{message}}',
    selectedFile: 'Selected: {{name}}',
    savingProfileStatus: 'Saving profile...',
    selectImageFirst: 'Select an image file first.',
    allowedTypesOnly: 'Only JPG, PNG, WEBP, or GIF images are allowed.',
    maxAvatarSize: 'Avatar file must be 5MB or smaller.',
    uploadingAvatarStatus: 'Uploading avatar...',
    uploadedButPreviewFailed: 'Uploaded, but preview failed: {{message}}',
    avatarUploadedAndSaved: 'Avatar uploaded and saved.',
    noAvatarToDelete: 'There is no avatar to delete.',
    deletingAvatarStatus: 'Deleting avatar...',
    avatarDeleted: 'Avatar deleted.',
    enterNewPassword: 'Please enter your new password.',
    passwordMinLength: 'Password must be at least 8 characters long.',
    passwordConfirmationMismatch: 'Password confirmation does not match.',
    updatingPasswordStatus: 'Updating password...',
    passwordUpdated: 'Password updated successfully.',
  },
  timeline: {
    unknownDate: 'Unknown date',
    unknownYear: 'Unknown year',
    dateTba: 'Date TBA',
    yearTba: 'Year TBA',
    issueLabel: 'Issue #{{number}}',
    heroTimelineTitle: '{{heroName}} timeline',
    signInToTrackIssues: 'Sign in to track which issues you own or have read.',
    syncingIssueStates: 'Syncing your issue states...',
    issueStateSyncError: 'Unable to sync your issue states. Please try again.',
    orderMode: 'Order:',
    canonicalOrder: 'Canonical',
    publicationOrder: 'Publication',
    sortByDate: 'Sort by date:',
    showcaseMode: 'Showcase mode',
    enterFullscreen: 'Fullscreen',
    exitFullscreen: 'Exit fullscreen',
    zoomTimeline: 'Zoom timeline:',
    zoomOutTimeline: 'Zoom out timeline',
    zoomInTimeline: 'Zoom in timeline',
    filterIssues: 'Filter issues:',
    filterPublication: 'Filter publication:',
    filterCollection: 'Filter collection:',
    newestFirst: 'Newest first',
    oldestFirst: 'Oldest first',
    allIssues: 'All issues',
    annualsOnly: 'Annuals only',
    allTracked: 'All',
    unreadOnly: 'Unread only',
    unownedOnly: 'Unowned only',
    readOnly: 'Read only',
    ownedOnly: 'Owned only',
    indexMonths: 'Months',
    indexYears: 'Years',
    indexStages: 'Stages',
    indexIssues: 'Issues',
    jumpTo: 'Jump to',
    hideTimelineIndex: 'Hide timeline index',
    noAnchorsForView: 'No anchors for this view.',
    showTimelineIndex: 'Show timeline index',
    stageLabel: 'Stage {{label}}',
    addToCollection: 'Add to collection',
    markAsRead: 'Mark as read',
    savingUpdate: 'Saving your update...',
    issueActionsUnavailable: 'Issue actions unavailable',
    issueStatesUnavailable: 'Issue states unavailable',
    saving: 'Saving...',
    haveIt: 'Have it',
    readIt: 'Read it',
    have: 'Have',
    read: 'Read',
    legacy: 'Legacy',
    specialEventMarker: 'Special event',
    specialEventFallbackTitle: 'Timeline milestone',
    noCover: 'No cover',
    coverAvailableSoon: 'Available soon',
    series: 'Series:',
    issue: 'Issue:',
    volume: 'Volume:',
    publication: 'Publication:',
    price: 'Price:',
    pages: 'Pages:',
    editing: 'Editing:',
    rating: 'Rating:',
    viewOnComicsOrg: 'View on comics.org',
    openCoverMobile: 'Open cover in fullscreen on mobile',
    coverViewer: 'Cover viewer',
    closeCoverViewer: 'Close cover viewer',
    coverPreviewError: 'Unable to load image preview.',
    missingHeroSlug: 'Missing hero slug. Timeline data cannot be requested yet.',
    configureBackendForTimeline: 'Set VITE_BACKEND_URL in your environment to enable hero timelines.',
    loadingTimeline: 'Loading timeline...',
    noIssuesLogged: 'No issues have been logged for this hero yet.',
    noAnnualIssues:
      'No annual issues are available for this hero. Switch back to "All issues" to view the complete timeline.',
    noIssuesForFilter: 'No issues match the selected filter.',
    signInToTrackCollection: 'Sign in to track your collection.',
    issueStateSyncUnavailable: 'Issue state sync is unavailable right now.',
    cinematicTimeline: 'Cinematic timeline',
    groupedByYear: 'Grouped by publication year',
    publicationYear: 'Publication year',
    sort: 'Sort',
    coverTbd: 'Cover TBD',
    addOneInSupabase: 'Add one in Supabase',
    noPrefix: 'No. {{number}}',
    release: 'Release',
    viewIssue: 'View issue',
    publishingResume: 'Publishing resume',
    recordedSaga: '{{heroName}}\'s recorded saga',
    recordedSagaFallback: 'Recorded hero saga',
    loadingOverview: 'Loading timeline overview...',
    overviewSentence:
      'We have {{totalIssues}} logged issues spanning {{rangeLabel}}. Explore every stage to understand the full reading order.',
    coverage: 'Coverage',
    trackedIssues: '{{count}} tracked issues',
    collectedEditionsCount: '{{count}} collected editions',
    collectionProgress: 'Collection progress',
    signInForCharts: 'Sign in to view collection progress charts.',
    signInToTrackOwnedRead: 'Sign in to track what you own and have finished reading.',
    syncingCollection: 'Syncing your collection...',
    stageMetadataMissing: 'Stage metadata has not been added yet. Entries will appear here once stages are tagged.',
    uncategorizedStage: 'Uncategorized stage',
    issueFallback: 'Issue',
    stageComplete: 'Stage · Complete',
    stage: 'Stage',
    noStageSummary: 'No summary has been added for this stage yet.',
    years: 'Years:',
    issuesTracked: 'Issues tracked:',
    stageIssues: 'Stage issues',
    noIssueItemsQuickActions: 'No issue items available for quick actions.',
    signInEnableQuickActions: 'Sign in to enable quick issue actions.',
    noTrackableIssues: 'No trackable issues in this stage yet.',
    allIssuesRead: 'All issues already marked as read.',
    readProgressUnavailable: 'Read progress unavailable',
    markStageAsRead: 'Mark stage as read',
    viewStageDetails: 'View stage details',
    marking: 'Marking...',
    allRead: 'All read',
    unableMarkStageRead: 'Unable to mark this stage as read.',
    unableUpdateIssueState: 'Unable to update issue state.',
    configureBackendForInsights: 'Configure VITE_BACKEND_URL to view publishing insights for this hero.',
    percentReadTitle: '{{percent}}% read',
    issuesCountOfTotal: '{{count}} / {{total}} issues',
    collectedEditionsTab: 'Omnibus and collected editions',
    collectedCoverageTitle: 'Collected coverage',
    collectedCoverageSubtitle: 'Collected editions and their contained issues across timeline stages.',
    collectedCoverageEmpty: 'No collected editions linked yet for this hero.',
    collectedLanguageFilter: 'Language',
    collectedFormatFilter: 'Collected type',
    collectedAllLanguages: 'All languages',
    collectedAllFormats: 'All types',
    collectedFilteredCount: '{{count}} of {{total}} collected editions',
    collectedFormatGroupCount: '{{count}} editions',
    collectedFilteredEmpty: 'No collected editions match these filters.',
    collectedIssuesCount: '{{count}} linked issues',
    collectedStagesCount: '{{count}} covered stages',
  },
  issueDetails: {
    title: 'Issue details',
    eyebrow: 'Issue details',
    closeDialog: 'Close issue details dialog',
    timelineNavigation: 'Timeline issue navigation',
    previousIssue: 'Previous timeline issue',
    nextIssue: 'Next timeline issue',
    loadError: 'Unable to load issue details right now.',
    retry: 'Retry',
    empty: 'Issue details are not available yet.',
    coverMissing: 'No cover image is available for this issue yet.',
    summaryLabel: 'Summary',
    noMetadata: 'No extra metadata is available for this issue yet.',
    openSource: 'Open source page',
    fields: {
      issueTitle: 'Issue title',
      issueNumber: 'Issue number',
      legacyNumber: 'Legacy number',
      seriesTitle: 'Series title',
      volume: 'Volume',
      issueDate: 'Issue date',
      publicationDate: 'Publication date',
      coverDate: 'Cover date',
      onSaleDate: 'On sale date',
      price: 'Price',
      pageCount: 'Page count',
      stage: 'Timeline stage',
      stageSummary: 'Stage summary',
      editing: 'Editing',
      rating: 'Rating',
      gcdIssueId: 'GCD issue id',
      timelineIssueId: 'Timeline issue id',
      severity: 'Severity',
    },
    collected: {
      title: 'Collected editions and reprints',
      description: 'This area is ready for future collected-format appearances of this issue.',
      placeholderTitle: 'Collected format placeholder',
      placeholderBody: 'No collected edition data has been connected yet.',
    },
    ownershipDialog: {
      eyebrow: 'Collection format',
      title: 'How do you own this issue?',
      singleIssueTitle: 'Single issue',
      singleIssueBody: 'Mark only this issue as part of your collection.',
      collectedEditionsLabel: 'Available collected editions',
      loading: 'Loading collected editions...',
      loadError: 'Unable to load collected editions for this issue.',
      noCollectedEditions: 'No collected editions are linked to this issue yet. You can still save ownership if you have it as a single issue.',
      confirm: 'Save ownership',
    },
  },
}

export default messages
